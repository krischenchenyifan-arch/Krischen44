...
class DifferentialEvolutionPeaks:
    def __init__(self, no_kids, no_var=2, no_gen=10, fr=1.0, sigma=1.0):
        ...
        self.history_dna = np.zeros((self.NO_GEN, self.NO_VAR))
        self.best_index = -1
    ...
    def run(self):
        ...
        for gen in range(self.NO_GEN):
            ...
            self.history_dna[gen, :] = self.kid_dna[self.best_index, :]

        return self.history_fitness, self.history_dna, self.kid_fitness[self.best_index], self.kid_dna[self.best_index]

# Main execution for N = 10 and N = 100
...
for no_kids in population_sizes:
    ...
    de = DifferentialEvolutionPeaks(no_kids=no_kids)
    history_fitness, history_dna, best_fitness, best_dna = de.run()
    
    results_fitness[no_kids] = history_fitness
    print(f"N = {no_kids} best fitness = {best_fitness} at ({best_dna[0]}, {best_dna[1]})")
